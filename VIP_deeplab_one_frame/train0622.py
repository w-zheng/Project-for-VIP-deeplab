...
class Trainer:
    ...
    def loss_instance(self,center_heatmap_pred,center_heatmap_gt,offset_pred,offset_gt,instance_mask,mask_center,mask_offset):
        ...
        center_loss = F.mse_loss(center_heatmap_pred, center_heatmap_gt, reduction="none")
    ...
    def train_epoch(self, dl: DataLoader):
        ...
        with tqdm(total=len(self.ds_split["train"]), desc=f"Training") as pbar:
            ...
            for inputs, truths in dl:
                ...
                center, offset = _generate_gt_center_and_offset(
                    batch_instance_labels=truth_instance
                )
                truth_instance = truth_instance.to(
                ...
                center_loss, offset_loss = self.loss_instance(
                    center_prediction,
                    center,
                    center_regression,
                    offset,
                ...
                # weighted loss
                loss = (
                    loss_depth + loss_seg + 200 * center_loss + 0.1 * offset_loss
                )
                loss.backward()
    ...
    def val_epoch(self, dl: DataLoader):
        ...
        with torch.no_grad(), tqdm(
            ...
            for inputs, truths in dl:
                ...
                # output_seg_2D = output_seg.argmax(dim=1) [10,64,64]
    ...
    #     num_train_samples = num_samples // 20
    #     num_val_samples = num_val // 20
    ...
    #         metrics_train = self.train_epoch(dl_train_subset)
    #         df_train = pd.concat(
    #             [
    #                 df_train,
    #                 pd.DataFrame(
    ...
    #         metrics_val = self.val_epoch(dl_val_subset)
    #         df_val = pd.concat(
    #             [
    #                 df_val,
    #                 pd.DataFrame(
...
# create folder
folder_name = "ViP_one_frame_Deep_0622"
...
painted1 = "absRel"
for i in range(1, train_results["epoch"].max()):
    val_loss_abs.append(val_results[val_results["epoch"] == i][painted1].mean(0))

plt.figure()
plt.plot(val_loss_abs, "r--")
plt.title(painted1)
plt.ylabel(painted1)
plt.xlabel("epoch")
plt.legend(["val"])
